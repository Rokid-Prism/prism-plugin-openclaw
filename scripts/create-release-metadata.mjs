...
const protocolVersion = get("protocol_version"), minHubVersion = get("min_hub_version");
...
await writeFile(resolve(artifacts, "plugin-release.json"), JSON.stringify({ schema_version: 1, plugin_id: id, version, release_tag: `v${version}`, channel: "stable", protocol_version: protocolVersion, min_hub_version: minHubVersion, supported_platforms: assets.map((asset) => asset.target), assets }, null, 2) + "\n");
